    ...
    foreach (array_merge(array_keys($group_map), [0]) as $group_position => $group_id) {
      ...
      foreach ($group_entries as $entry) {
        ...
        if (!$group) {
          ...
          echo '<div class="cfm-views-current-term-row" data-cfm-views-current-term data-uuid="' . esc_attr((string) $entry->term_uuid) . '" data-parent="' . esc_attr($entry_parent) . '" data-depth="' . esc_attr((string) $entry_depth) . '" style="margin-left:' . esc_attr((string) ($entry_depth * 24)) . 'px">' . ($entry_has_children ? '<button type="button" class="button-link cfm-views-current-toggle" data-cfm-views-current-toggle aria-expanded="true">▾</button>' : '<span class="cfm-views-current-toggle-spacer" aria-hidden="true"></span>') . '<label><input type="checkbox" form="cfm-views-remove-form" name="entry_ids[]" value="' . esc_attr((string) $entry->id) . '" data-cfm-views-entry-select aria-label="Select ' . esc_attr($canonical_label) . ' for removal"> <strong>' . esc_html($canonical_label) . '</strong></label></div>';
          continue;
        }
