...
class CFM_Views_Admin
{
  ...
  public static function handle_actions(): void
  {
    ...
    if ($action === 'create_view') {
      ...
      CFM_Views_Repository::save_entry($version_id, ['term_uuid' => wp_unslash($_POST['term_uuid'] ?? ''), 'core_terms_framework' => wp_unslash($_POST['framework'] ?? ''), 'inclusion' => sanitize_key(wp_unslash($_POST['inclusion'] ?? 'include')), 'display_label' => wp_unslash($_POST['display_label'] ?? ''), 'display_order' => absint($_POST['display_order'] ?? 0), 'include_descendants' => !empty($_POST['include_descendants'])]);
  ...
  private static function render_draft_editor(int $version_id): void
  {
    ...
    echo '<hr><h2>Draft composition: ' . esc_html($view ? $view->name : 'View') . '</h2><p>Draft-only workspace. Add canonical Core Terms to this View, then return to the list to validate and publish.</p>';
    ...
    echo '<h3>Draft entries</h3><table class="widefat striped"><thead><tr><th>Framework</th><th>Term UUID</th><th>Inclusion</th><th>Label</th><th>Order</th><th>Descendants</th><th>Action</th></tr></thead><tbody>';
    foreach ((array) $entries as $entry) {
      echo '<tr><td>' . esc_html((string) $entry->core_terms_framework) . '</td><td><code>' . esc_html((string) $entry->term_uuid) . '</code></td><td>' . esc_html((string) $entry->inclusion) . '</td><td>' . esc_html((string) ($entry->display_label ?: 'Canonical label')) . '</td><td>' . esc_html((string) $entry->display_order) . '</td><td>' . (!empty($entry->include_descendants) ? 'Yes' : 'No') . '</td><td><form method="post">';
    ...
    if (!$entries) { echo '<tr><td colspan="7">No terms added to this draft yet.</td></tr>'; }
    echo '</tbody></table>';
  }
}
