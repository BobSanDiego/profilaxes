...
class CFM_Views_Admin
{
  ...
  private static function render_views_library_nodes(array $nodes, array $children_by_parent, string $slug, array $represented_terms, int $depth): void
  {
    foreach ($nodes as $term) {
      ...
      echo ($depth > 0 ? '<input type="checkbox" name="term_uuids[]" value="' . esc_attr($slug . '|' . $uuid) . '" data-cfm-views-select aria-label="Select ' . esc_attr($label) . '"' . ($represented ? ' disabled' : '') . '>' : '<span class="cfm-views-top-level-marker" aria-hidden="true"></span>') . '<span class="cfm-views-term-label" data-cfm-views-term-name>' . esc_html($label) . '</span></div>';
  ...
  private static function render_workbench_assets(): void
  {
    ...
    echo '<style id="cfm-views-library-compact-styles">.cfm-views-source .cfm-views-term-tree{background:#f1f1f1;border:0;padding:8px 12px}.cfm-views-source .cfm-views-term-node{margin:0}.cfm-views-source .cfm-views-term-row{display:flex;align-items:center;gap:7px;min-height:28px;padding:1px 0;border:0;background:transparent}.cfm-views-source .cfm-views-term-label{padding-left:0;white-space:nowrap}.cfm-views-source .cfm-views-term-children{margin-left:20px}.cfm-views-source .cfm-views-toggle,.cfm-views-source .cfm-views-toggle-spacer{width:16px;min-width:16px;padding:0;text-align:center}.cfm-views-source .cfm-views-toggle{font-size:14px;line-height:18px}.cfm-views-source .cfm-views-term-row[data-cfm-views-hidden="true"]{display:none}</style>';
    echo '<script id="cfm-views-workbench-script">(function(){"use strict";var root=document.querySelector("[data-cfm-views-workbench]");if(!root){return;}var framework=root.querySelector("[data-cfm-views-framework]"),search=root.querySelector("[data-cfm-views-search]"),count=root.querySelector("[data-cfm-views-result-count]"),selectedCount=root.querySelector("[data-cfm-views-selected-count]"),selectVisible=root.querySelector("[data-cfm-views-select-visible]"),clearSelection=root.querySelector("[data-cfm-views-clear-selection]"),trees=[].slice.call(root.querySelectorAll("[data-cfm-views-tree]"));function activeTree(){return trees.find(function(tree){return tree.dataset.framework===framework.value;});}function refreshSelection(){var selected=root.querySelectorAll("[data-cfm-views-select]:checked").length;if(selectedCount){selectedCount.textContent=selected+" selected";}}function refresh(){var tree=activeTree(),needle=(search.value||"").toLowerCase().trim(),visible=0;if(!tree){return;}trees.forEach(function(item){item.hidden=item!==tree;});[].slice.call(tree.querySelectorAll("[data-cfm-views-term]")).forEach(function(row){var match=!needle||row.dataset.label.indexOf(needle)!==-1;row.dataset.cfmViewsMatch=match?"true":"false";if(match){visible++;}if(!row.hasAttribute("data-cfm-views-hidden")){row.dataset.cfmViewsHidden="false";}});if(count){count.textContent=visible+" canonical term"+(visible===1?"":"s");}}if(framework){framework.addEventListener("change",function(){search.value="";refresh();});}if(search){search.addEventListener("input",refresh);}root.addEventListener("change",function(event){if(event.target.matches("[data-cfm-views-select]")){refreshSelection();}});if(selectVisible){selectVisible.addEventListener("click",function(){var tree=activeTree();if(tree){tree.querySelectorAll("[data-cfm-views-term]").forEach(function(row){if(row.dataset.cfmViewsHidden!=="true"&&row.dataset.cfmViewsMatch!=="false"){var box=row.querySelector("[data-cfm-views-select]");if(box){box.checked=true;}}});refreshSelection();}});}if(clearSelection){clearSelection.addEventListener("click",function(){root.querySelectorAll("[data-cfm-views-select]").forEach(function(box){box.checked=false;});refreshSelection();});}root.addEventListener("click",function(event){var add=event.target.closest("[data-cfm-views-add]");if(add){var select=document.querySelector("#cfm-view-term");if(select){select.value=add.dataset.cfmViewsAdd;select.dispatchEvent(new Event("change",{bubbles:true}));select.focus();}}var toggle=event.target.closest("[data-cfm-views-toggle]");if(toggle){var row=toggle.closest("[data-cfm-views-term]"),tree=row.closest("[data-cfm-views-tree]"),collapsed=toggle.getAttribute("aria-expanded")==="true";toggle.setAttribute("aria-expanded",collapsed?"false":"true");toggle.textContent=collapsed?"+":"−";var hide=collapsed;[].slice.call(tree.querySelectorAll("[data-cfm-views-term]")).forEach(function(candidate){if(candidate===row){return;}var parent=candidate.dataset.parent;var ancestor=row.dataset.uuid;while(parent){if(parent===ancestor){candidate.dataset.cfmViewsHidden=hide?"true":"false";if(hide){return;}ancestor=parent;var parentRow=tree.querySelector("[data-uuid=\""+CSS.escape(parent)+"\"]");parent=parentRow?parentRow.dataset.parent:"";break;}var parentRow=tree.querySelector("[data-uuid=\""+CSS.escape(parent)+"\"]");parent=parentRow?parentRow.dataset.parent:"";}});}});refresh();refreshSelection();})();</script>';
